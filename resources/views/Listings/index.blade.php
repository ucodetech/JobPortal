            ...
            <div class="flex-justify-center">
                @foreach ($tags as $tag)
                    <a href="{{ route('listings.index', ['tag'=>$tag->slug]) }}"
                        class="inline-block ml-2 tracking-wide text-xs font-medium title-font py-1 px-2 mb-2 border text-gray-900 border-indigo-500 rounded-md uppercase {{ $tag->slug === request()->tag ? ' bg-indigo-500 text-white' : 'bg-white text-indigo-500' }}">
                        {{ $tag->name }}
                    </a>
                @endforeach
            </div>
        ...
        <div class="my-6">
            ...
            @foreach ($listings as $listing)
                ...
                >
                    ...
                    <span class="md:flex-grow flex items-center justify-end text-emerald-950">
